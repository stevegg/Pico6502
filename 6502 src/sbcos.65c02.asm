...
;
Input_chr:
               jmp   ACIA1_Input       ; wait for input character
Scan_input:
               jmp   ACIA1_Scan        ; scan for input (no wait), C=1 char, C=0 no character
Output:
               jmp   ACIA1_Output      ; send 1 character
;
;               
; *************************************************************************
; kernal commands
...
Print2Byte:
               JSR   Print1Byte        ;  prints AAXX hex digits
               TXA                     ;
Print1Byte:
               ...
               PLA                     ;
PrintDig:
               sty   ysav              ;  prints A hex nibble (low 4 bits)
               AND   #$0F              ;
               TAY                     ;
               LDA   Hexdigdata,Y      ;
               ldy   ysav              ;
...
Mon08:
               CMP   CmdAscii,X        ;  is non hex cmd chr?
...
CR_cmd:
               ...
               jmp   SP_cmd2           ;
SP_cmd:
               LDA   Hexdigcnt         ; Space command entry
...
SP_cmd5:
               RTS                     ; done or no digits to process
;
TXT_Cmd:
               sty   ysav              ;
               ldy   #$00              ;
